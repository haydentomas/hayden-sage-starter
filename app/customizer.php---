...
add_action('customize_register', function (\WP_Customize_Manager $wp_customize) {
    ...
    $active_when = function ($setting_id, $equals) {
    ...
    $wp_customize->add_setting('hayden_header_layout', [
        'default'           => 'default',
        'transport'         => 'refresh',
        'sanitize_callback' => $sanitize_select(['default', 'logo-top', 'nav-center-cta'], 'default'),
    ]);
    ...
    // CTA label (only visible when layout = nav-center-cta)
    ...
    // CTA URL (only visible when layout = nav-center-cta)
    ...
    // Widget title/text/link colours
    ...
    // Subheading: Footer widget colours
     ...
     * Adds: Footer Variant + Footer Columns
     * ------------------------------------------------------------
     */
    ...
    // Footer variant (A/B/C/Default) - you will later wire this to your Blade includes
    ...
    // Footer content page (used when footer variant = none)
...
$wp_customize->add_control('hayden_footer_content_page_id_control', [
    ...
    'active_callback' => function ($control) {
        // Adjust this if your setting name differs:
        $variant = $control->manager->get_setting('hayden_footer_variant')?->value();
        return $variant === 'none';
    },
]);
...
$wp_customize->add_control("{$setting_id}_control", [
  ...
  'active_callback' => function ($control) use ($active_when_footer_variant) {
      // Only when Footer C is selected AND CTA enabled
      $is_footer_c = $active_when_footer_variant('footer-c')($control);
      $cta_on = (bool) $control->manager->get_setting('hayden_footer_c_cta_enabled')?->value();
...
/**
 * ------------------------------------------------------------
 * Editor: inject theme CSS vars (basic subset)
 * ------------------------------------------------------------
 */
add_action('enqueue_block_editor_assets', function () {
    ...
    $css = sprintf(
        ':root{--color-primary:%1$s;--color-surface:%2$s;--card-bg:%3$s;--card-heading:%4$s;--card-text:%5$s;--card-text-muted:%6$s;--section-space-mobile:%7$s;--section-space-desktop:%8$s;}',
        esc_html($primary),
        esc_html($surface),
        esc_html($card_bg),
...
/**
 * Customizer controls styling + JS (controls frame)
 */
add_action('customize_controls_enqueue_scripts', function () {
    ...
    $accent_css = sprintf(':root { --color-primary: %s; }', esc_attr($primary));
...
(function(api) {
  ...
  api('hayden_primary_color', function(setting) {
    ...
    setting.bind(function(newVal) { applyPrimary(newVal); });
  });
...
/**
 * Ensure editor also gets font CSS + spacing vars.
 */
add_action('enqueue_block_editor_assets', function () {
    $css = '';
    ...
    $css .= "
    :root{--section-space-mobile:{$spacing['mobile']};--section-space-desktop:{$spacing['desktop']};}
